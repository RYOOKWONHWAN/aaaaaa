...
def find_movieId(_input_movies, _movies_df):
    ...
    for movie in _input_movies:
        index = _movies_df.loc[_movies_df["title"]
                               == movie, 'id'].values[0]
...
def do_apriori(_input_movies, _movies_df, _ratings_df):    # 제목과 평점으로 분석해서 추천
    # Internal variables
    print("aprio :")
    print(_input_movies)
    print(_movies_df.head())
    print(_ratings_df.head())
    _apriori_result = []
    ...
    _movies_df = _movies_df.astype({'movie_id': 'int64'})
    df = pd.merge(
        _ratings_df, _movies_df[['movie_id', 'title']], left_on='movieId', right_on='movie_id')
    df.drop(['timestamp', 'movie_id'], axis=1, inplace=True)
    print(df.head())
    """ Prepare Apriori
        row : userId | col : movies """
    df = df.drop_duplicates(['userId', 'title'])
    df_pivot = df.pivot(index='userId', columns='title',
    ...
    frequent_items = apriori(
        df_pivot, min_support=0.0197, use_colnames=True)
    print("frequent")
    print(frequent_items.head())

    # using association rules, compute the other parameter ex) confidence, lift ..
    association_indicator = association_rules(
        frequent_items, metric="lift", min_threshold=0.8)
    print("========================================")
    print("association")
    print(association_indicator)
    # sort by order of lift
    df_lift = association_indicator.sort_values(
        by=['lift'], ascending=False)  # lift 는 분석해서 계산한 결과값  1 이면 보통 상관관계가 있다
    print(df_lift)

    """ Start recommendation """
    for selected_movie in _input_movies:
        num = 0
        df_selected = df_lift[df_lift['antecedents'].apply(
            lambda x: len(x) == 1 and next(iter(x)) == selected_movie)]
        print("=======================df selected.head==========================")
        print(df_selected.head())
        # 보통 1.2~2.0을 사용해서 추천함
...
def do_kmeans(_apriori_result, _input_movies, _movies_df):
    ...
    numeric_df = _movies_df[['popularity',
                             'runtime', 'vote_average', 'vote_count', 'title', '16', '12', '10751', '14', '35', '28', '878', '18', '9648', '27', '53', '80', '36', '10752', '10749', '99', '10402', '10770', '37']]   # 인기도 런타임 투표 평균점수 투표 수를 토대로 비슷한 영화를 군집으로 묶는다.
    ...
    minmax_processed = preprocessing.MinMaxScaler(
    ).fit_transform(df_numeric.drop('title', axis=1))
    ...
    score = [kmeans[i].fit(df_numeric_scaled).score(
        df_numeric_scaled) for i in range(len(kmeans))]
    ...
    for movie1 in _input_movies:
        try:
            cluster_candid = df_numeric.loc[df_numeric["title"]
                                            == movie1, 'cluster'].values[0]
    ...
    for movie2 in _apriori_result:
        try:
            cluster_tmp = df_numeric.loc[df_numeric["title"]
                                         == movie2, 'cluster'].values[0]
...
def compute_CF(_movie, _movies_users_compressed, _model, _n_recommendations, _movie_names):
    ...
    try:
        ...
        distances, indices = _model.kneighbors(
            _movies_users_compressed[movie_index], n_neighbors=_n_recommendations)
    except IndexError as e:
    ...
    for index in rec_indices:
        recommend_frame.append(
            {'Title': _movie_names['title'][index[0]], 'Distance': index[1]})
...
def do_collaborative_filtering(_input_movies, _ratings_df, _movies_df):
    ...
    movie_names = _movies_df[['title']]

    movies_users = ratings_data.pivot(index=['userId'], columns=[
                                      'movieId'], values='rating').fillna(0)
    ...
    for movie in _input_movies:
        num_recom = 4
        recommend = compute_CF(
            movie, movies_users_compressed, model, num_recom, movie_names)
...
def main(input_movies):
    ...
    """
    final_result += "Selected movies (5 movies) : " + \
        ",".join(input_movies) + "\n\n"
    print(final_result)
    ...
    ratings_df = pd.read_csv('python/ratings_small2.csv')
    ...
    collabo_result = do_collaborative_filtering(
        input_movies, ratings_df, movies_df)

    final_result += "A-priori & K-means clustering recommend movie : " + \
        ",".join(kmeans_result) + "\n\n"
    final_result += "Collaborative filtering recommend movie : " + \
        ",".join(collabo_result) + "\n\n"
